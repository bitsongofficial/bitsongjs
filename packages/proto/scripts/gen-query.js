...
(async () => {
  ...
  await exec(`echo '// See scripts/gen-query.js' >> "${outputFile}"`);

  const { stdout } = await exec(`find "${__dirname}/../src" -name query.ts`);
  const modulesPaths = stdout
    .split("\n")
    .filter((modulePath) => modulePath.length > 0)
    .filter((modulePath) => !modulePath.includes("/bitsong/"))
